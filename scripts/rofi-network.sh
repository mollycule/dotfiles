...
if [ $choice == "CANCEL" ]; then
    ...
    if ! pgrep openvpn; then
        ...
        if ! sudo kill $(pgrep openvpn); then
            notify-send -u normal "NET" "Unable to stop openvpn." &
            exit 1
        fi
    fi
    ...
    if ! sudo netctl start $choice; then
        ...
        if echo $choice | grep -q "vpn"; then
            ...
            if ! ps -u root | grep -q "openvpn"; then
                notify-send -u normal "NET" "Unable to start vpn profile." &
                exit 1
            else
                notify-send -u normal "NET" "Successfully started vpn profile." &
                exit 0
            fi
        else
