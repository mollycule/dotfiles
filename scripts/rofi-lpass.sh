...
if [ "$output" = "ERROR" ]; then
    ...
    notify-send -u low "i3" "Action cancelled [1]." &
elif [ "$output" = "" ]; then
    notify-send -u low "i3" "Action cancelled [2]." &
elif [ "$output" = "LOGOUT" ]; then
    notify-send -u low "i3" "Successfully logged-out." &
else
    echo -n "$output" | xclip -sel clip
    notify-send -u low "i3" "Password copied to clipboard." &
fi
